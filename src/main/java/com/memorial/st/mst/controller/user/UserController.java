...
public class UserController {
    ...
    @AuthExcludes
    @PostMapping("/login")
    public ResponseEntity<Boolean> login(@RequestBody MstUserRequest request, HttpServletResponse response) throws Exception {
        try {
    ...
    public ResponseEntity<MstUserResponse> register(@RequestBody MstUserRequest request, HttpServletResponse response) throws Exception {
        return ResponseEntity.ok(new MstUserResponse(userService.register(request.getUserName(), request.getNickName(), request.getPassword(), request.getRole())));
    }

    // 사용자 정보 동의
    @PostMapping("/consent")
    public ResponseEntity<Boolean> createConsent(@RequestBody UserConsentRequest consentRequest) {
